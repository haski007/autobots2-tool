...
async function main() {
  ...
  for (const bot in skinBotMap) {
    ...
    for (const trade of trades) {
      const tradeSkins = await mongo.find('skins', { _trade: trade._id })
      if (trade.itemsToReceive.length - tradeSkins.length !== 0) tradesToCheck.push(trade)
    }
  }
  
  console.log('Trades to check', tradesToCheck.length)
  ...
  for (const trade of tradesToCheck) {
    ...
    if (offer && offer.trade_offer_state === 3) {
      ...
      const ourSteamId = (await mongo.find('bots', { apiKey: trade.apiKey }))[0].steamId
      if (items && items.length)
        for (const item of items) {
          const mongoItem = (await mongo.find('skins', { assetid: item.new_assetid }))[0]
          if (!mongoItem) {
            console.log('Item not found:', item.new_assetid)
          } else await mongo.update('skins', { assetid: item.new_assetid }, {
